...
// 저장 키
  ...
  Future<void> _saveDeckToLocal() async {
    ...
    await prefs.setBool('union_deck_builder_allow_unowned', _allowUnowned);
  }
  ...
  Future<void> _loadDeckFromLocal() async {
    ...
    final savedAllowUnowned = prefs.getBool('union_deck_builder_allow_unowned');
  ...
  void _exportDeckCode() {
    ...
    final code = DeckCodeUtils.encodeDeck(
        type: 'union', squads: squadCodes, elements: _squadNames);
    if (code.isNotEmpty) {
  ...
  void _importDeckCode(String code) {
    ...
    } else if (result != null && result.type == 'solo') {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('솔로레이드 덱입니다! 유니온레이드 덱 빌더에서는 불러올 수 없습니다.')),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
